...
datExpr = as.data.frame(t(orthologs_expressed[, c(54:56,51:53,57:59,45:47,42:44,48:50)]));
names(datExpr) = orthologs_expressed$Sv_geneID;
dim(datExpr) 
rownames(datExpr)
colnames(datExpr)
...
#pdf("Sv_MEtree.pdf", width = 6, height = 5)
plot(METree, main = "Clustering of module eigengenes",
     xlab = "", sub = "")
...
#pdf("Sv_Dendrogram.pdf", width = 5, height = 3)
...
save(MEs, moduleLabels, moduleColors, geneTree, file = "ME34-orthologs.RData")
...
for (i in c(1:lumc)){
  ...
  pdf_file_out= paste("ME34-total-nostate-30-0.14-",umc[i],".pdf",sep="")
...
###### Step 3 ###### Exporting network data for network visualization 

{
  ...
  cyt = exportNetworkToCytoscape(modTOM,
                                 edgeFile = paste("ME34-CytoscapeInput-edges-", paste(module, collapse="-"), ".txt", sep=""),
                                 nodeFile = paste("ME34-CytoscapeInput-nodes-", paste(module, collapse="-"), ".txt", sep=""),
                                 weighted = TRUE,
                                 threshold = 0,
                                 nodeNames = modProbes,
...
cyt = exportNetworkToCytoscape(TOM,
                               edgeFile = paste("ME34-CytoscapeInput-edges-th02.txt", sep=""),
                               nodeFile = paste("ME34-CytoscapeInput-nodes-th02.txt", sep=""),
                               weighted = TRUE,
                               threshold = 0.2,
                               nodeNames = probes,
