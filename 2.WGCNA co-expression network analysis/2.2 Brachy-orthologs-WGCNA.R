...
datExpr = as.data.frame(t(orthologs_expressed[, c(18:20,15:17,21:23,9:11,6:8,12:14)]));
names(datExpr) = orthologs_expressed$Bd_geneID;
dim(datExpr) 
rownames(datExpr)
colnames(datExpr)
...
pdf(file = "Bd_sampleClustering.pdf", width = 12, height = 9);
par(cex = 0.6);
par(mar = c(0,4,2,0))
...
plot(METree, main = "Clustering of module eigengenes",
     xlab = "", sub = "")
#pdf("Bd_MEtree.pdf", width = 6, height = 5)
plot(METree, main = "Clustering of module eigengenes",
     xlab = "", sub = "")
...
#pdf("Bd_Dendrogram.pdf", width = 5, height = 3)
...
save(MEs, moduleLabels, moduleColors, geneTree, file = "Bd-orthologs.RData")
...
for (i in c(1:lumc)){
  ...
  pdf_file_out= paste("Bd-total-nostate-30-0.16-",umc[i],".pdf",sep="")
...
for (module in unique(mergedColors))
{
  ...
  cyt = exportNetworkToCytoscape(modTOM,
                                 edgeFile = paste("Bd-CytoscapeInput-edges-", paste(module, collapse="-"), ".txt", sep=""),
                                 nodeFile = paste("Bd-CytoscapeInput-nodes-", paste(module, collapse="-"), ".txt", sep=""),
                                 weighted = TRUE,
                                 threshold = 0,
                                 nodeNames = modProbes,
...
cyt = exportNetworkToCytoscape(TOM,
                               edgeFile = paste("Bd-CytoscapeInput-edges-th02.txt", sep=""),
                              nodeFile = paste("Bd-CytoscapeInput-nodes-th02.txt", sep=""),
                               weighted = TRUE,
                               threshold = 0.1,
                               nodeNames = probes,
...
write.csv(KIM, file = "Bd-total-IntramodularConnectivity.csv")